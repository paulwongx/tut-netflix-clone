...
const PlansScreen = () => {
	...
	const loadCheckout = async priceId => {
		...
		docRef.onSnapshot(async snap => {
			...
			if (error) {
				// Show an error to your customer nad
			...
			if (sessionId) {
				...
				const stripe = await loadStripe(
					'pk_test_hgh7czHD3HoyRMXtJBLdidV8'
				);
	...
	return (
		<div className='plansScreen'>
            {subscription && (<p>Renewal data: {new Date(subscription?.current_period_end * 1000).toLocaleDateString()}</p>)}
